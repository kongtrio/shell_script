...
for ip in $(cat $node_ip_path); do
    ...
    for ((instance = 0; instance < 4; instance++)); do
        ...
        if [[ $action == "start" ]]; then
            ssh $ip "/www/c4_presto_$instance/bin/launcher $action -Dwork_seq=$instance --server-log-file /var/log/Bigdata/presto/worker$instance/server.log"
        else
            ssh $ip "/www/c4_presto_$instance/bin/launcher $action"
        fi
