...
for ip in $(cat $node_ip_path); do
    ...
    for ((instance = 0; instance < 4; instance++)); do
        ...
        echo "$ip ${instance_name} execute $action..."
        if [[ $action == "start" ]]; then
            ssh $ip "/www/${instance_name}/bin/launcher $action -Dwork_seq=$instance --server-log-file /var/log/Bigdata/presto/${instance_log_dir_name}/server.log"
        else
            ssh $ip "/www/${instance_name}/bin/launcher $action"
        fi
