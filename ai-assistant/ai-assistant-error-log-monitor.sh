...
# 日志存放目录
log_path=/www/ai-assistant-api/logs
# 日志路径
...
check_error() {
    ...
    if [ "${now_hour}" == "00" ];then
        error_count=$(zcat ${yeaterday_file_path} | sed -n "/^${check_begin_time}/,/^${check_end_time}/p" | egrep "${error_match_pattern}" |  wc -l)
    else
        error_count=$(cat ${log_file_path} | sed -n "/^${check_begin_time}/,/^${check_end_time}/p" | egrep "${error_match_pattern}" |  wc -l)
    fi
