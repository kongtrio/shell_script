...
base_dir="/www"
server_home="bistoury-agent-bin"
full_server_path=${base_dir}/${server_home}
jar_name="bistoury-agent-bin.tar.gz"

stop_server() {
    #停止服务
    echo "--------------执行关闭服务操作"
    sh ${full_server_path}/bin/bistoury-agent.sh stop
    if [ $? -ne 0 ]; then
        echo "停止bistoury agent服务失败"
    else
        echo "停止bistoury agent服务成功"
    fi
}
...
stopServer)
    ...
    ;;
*)
    echo "$0:usage: update | rollback | stopServer"
    exit 1
    ;;
esac
